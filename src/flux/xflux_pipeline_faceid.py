...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False, enable_redux_prior: bool = True):
        ...
        if enable_redux_prior:  
            self.set_redux_prior()

    def set_redux_prior(self, siglip_ckpt: str = "google/siglip2-so400m-patch16-512"):
        ...
        self.pipe_prior_redux = FluxPriorReduxPipeline(
    ...
    def __call__(self,
        ...
        return self.forward(
            ...
            input_ref_image=input_ref_image,
        )
    ...
    def forward(
        ...
        ref_image = load_image(input_ref_image)
        # import pdb; pdb.set_trace()
        pipe_prior_output = self.pipe_prior_redux(ref_image)
        prior_prompt_embeds = pipe_prior_output.prompt_embeds
        ...
        with torch.no_grad():
            ...
            inp_cond['txt'] = torch.cat([inp_cond['txt'], prior_prompt_embeds], dim=1)
            B,L,C = inp_cond['txt'].shape
            inp_cond['txt_ids'] = torch.zeros((B, L, 3), dtype=inp_cond['txt_ids'].dtype, device=inp_cond['txt_ids'].device)
...
class XFluxSampler(XFluxPipeline):
    def __init__(self, clip, t5, ae, model, device):
        ...
        self.ip_loaded = False
        self.offload = False
        # 确保 Redux 先验被初始化，从而提供 pipe_prior_redux
        self.set_redux_prior()
